...
export async function POST(request: Request) {
  ...
  if (!isKitConfigured()) {
    ...
    console.warn("[free-class] Kit not configured — skipping subscriber sync.");
  ...
  try {
    ...
    await subscribeToFreeClassForm({
      email,
      firstName,
      referrer: marketingUrl ? `${marketingUrl}/free-class` : undefined,
    });
    return NextResponse.json({ ok: true });
  } catch (error) {
    console.error("[free-class] Kit subscribe failed:", error);
    return NextResponse.json(
